...
const runExpiration = async () => {
  ...
  try {
    // Use the Redis pool to acquire a client for this operation
    const res = await withRedisClient(async (client) => {
      return await client.set(
        EXPIRATION_CACHE_KEY, 
        1, 
        'EX', 
        EXPIRATION_LOCK_DURATION_SEC, 
        'NX'
      );
    });
  ...
  setTimeout(
    runExpiration, 
    EXPIRATION_TIMEOUT_MSEC - (Date.now() - startTime)
  );
};
...
const runOrgUsage = async () => {
  ...
  try {
    ...
    logger.error('Error in org usage tracking job:', error);
  }
  
  // Schedule the next run
  setTimeout(
    runOrgUsage, 
    ORG_USAGE_TIMEOUT_MSEC - (Date.now() - startTime)
  );
};

// Schedule first run at 3am
...
const gracefulShutdown = async () => {
  ...
  // Example: Close Redis pool when process is exiting
  await import('lib/connections/redis').then(redis => redis.closePool());
