...
const runBatchDelete = async ({
  ...
  try {
    ...
    if (batchStatementDeletionLockTimoutSec) {
      // Use the Redis pool to acquire a client for this operation
      res = await withRedisClient(async (client) => {
        return client.set(
    ...
    logger.error('Error in batch delete job:', error);
    
    // Even if there's an error, schedule the next run
    // Default to running again in 5 minutes if we can't determine the next window
    const retryInMs = 5 * 60 * 1000;
    logger.debug(`Error occurred, retrying in 5 minutes`);
    setTimeout(() => runBatchDelete({}), retryInMs);
